    ...
    (session, args) => {
      ...
      if (args.hypothesis_update) {
        ...
        if (update.hypothesis_id) {
          const hyp = session.data.hypotheses.find(
            (h: any) => h.id === update.hypothesis_id,
          );
          if (hyp) {
            hyp.linkedThoughts ??= [];
            hyp.linkedThoughts.push(session.stepNumber);
            if (update.new_hypothesis_text)
              hyp.text = update.new_hypothesis_text;
...
const inputSchema = z.object({
  problem: z
    ...
    .optional()
    .describe(
      "The exact question or problem to investigate—required on first call only",
    ),
  step: z
    .enum([
  ...
  content: z
    .string()
    .describe("Clear, focused contribution to the chosen step"),
  targets_hypothesis_id: z
    .array(z.string())
    .optional()
    .describe(
      "Hypothesis ID(s) this step references (e.g., ['h1'])—required for prediction, testing, analysis",
    ),
  hypothesis_update: z
    .object({
      hypothesis_id: z
        .string()
        .optional()
        .describe("Existing ID to modify/refute (omit for new hypothesis)"),
      new_hypothesis_text: z
        .string()
        .optional()
        .describe("Text of a new or revised hypothesis (must be testable)"),
      action: z
        .enum(["propose", "refine", "refute", "support"])
        .optional()
        .describe("Action taken on the hypothesis in this step"),
    })
    .optional()
    .describe(
      "Create or update a hypothesis in hypothesis_formulation or analysis steps",
    ),
  nextThoughtNeeded: z
    .boolean()
    .describe(
      "False only when conclusion step provides a final, evidence-based answer",
    ),
  final_answer: z
    .string()
    .optional()
    .describe(
      "The concluded answer—required when nextThoughtNeeded is false; must be justified by prior steps",
    ),
});
