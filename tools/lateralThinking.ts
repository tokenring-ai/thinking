...
function execute(
  ...
  return thinkingService.processStep(name, args, agent, (session, args) => {
    ...
    if (args.step === "generate_stimulus")
      session.data.stimuli.push(args.content);
    if (args.step === "force_connection")
      session.data.connections.push(args.content);
    if (args.step === "extract_insight")
      session.data.insights.push(args.content);
    if (args.step === "apply_to_problem")
      session.data.application = args.content;
